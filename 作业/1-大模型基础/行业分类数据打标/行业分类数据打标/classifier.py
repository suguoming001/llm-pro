...
# --- 日志配置 ---
def setup_logger(log_file_name: str = "eval_log.log"):
    """配置日志系统：同时输出到文件和控制台"""
...
logger = setup_logger(f"eval_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
def run_evaluation(solver: TagSolveSolver, tasks: List[Task]):
    results = []
    correct_count = 0
