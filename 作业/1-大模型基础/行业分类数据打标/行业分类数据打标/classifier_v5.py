...
class TagSolveSolver:
    ...
    def solve(self, task: Task) -> Dict[str, Any]:
        ...
        answers = []
        answers_reasoning_dict = dict()
        for _ in range(self.n_samples):
            raw = self.llm.generate(messages, max_tokens=20000)
            logger.info(f"模型输出 prompt is {messages} response is {raw}")

            result = self._parse(raw)
            result["_input"] = text
            answer = result.get('tag', '')
            answers.append(answer)
            answers_reasoning_dict[answer] = result

        final_answer = Counter(answers).most_common(1)[0][0]
        final_reasonings = answers_reasoning_dict[final_answer]
...
def run_evaluation(solver: TagSolveSolver, tasks: List[Task], max_workers: int = 8,
                   output_file="./output/result.xlsx"):
    total_count = len(tasks)
    total_start_time = datetime.now()
    logger.info(f"开始评测，共 {total_count} 条数据，并发数: {max_workers} ...")

    # 用字典收集结果，key 为原始索引，保证顺序
    results_dict: Dict[int, Dict[str, Any]] = {}
    correct_count = 0
    completed_count = 0
    lock = threading.Lock()

    def process_one(index: int, task: Task):
        nonlocal correct_count, completed_count
        start_time = datetime.now()
        try:
            res = solver.solve(task)
        except Exception as e:
            logger.error(f"第 {index + 1} 条处理异常: {e}")
            res = {
                "data": task.data,
        ...
        with lock:
            results_dict[index] = res
            completed_count += 1
            if res['correct']:
                correct_count += 1

            current_accuracy = correct_count / completed_count * 100
            logger.info(f"[{completed_count}/{total_count}] 第 {index + 1} 条完成")
            logger.info(f"  正确记录: {correct_count}  总数: {completed_count}  错误: {completed_count - correct_count}  准确率: {current_accuracy:.2f}%")
            logger.info(f"  正确tag: {res['ground_truth']}  预测tag: {res['predicted_answer']}")
            logger.info(f"  耗时: {latency.total_seconds():.2f}s")
            logger.info("-" * 50)

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(process_one, i, task): i
        ...
        for future in as_completed(futures):
            try:
                ...
                idx = futures[future]
                logger.error(f"第 {idx + 1} 条未捕获异常: {e}")

    # 按原始顺序组装结果
    results = [results_dict[i] for i in range(total_count)]
...
if __name__ == '__main__':
    ...
    run_evaluation(solver, task_list, max_workers=8, output_file='./output/result-v5.xlsx')